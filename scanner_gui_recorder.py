...
DSDPLUS_ROOT = os.environ.get("XSCAN_DSDPLUS_ROOT", os.path.join(LOCAL_APP_DATA, "Programs", "DSDPlus"))
LOG_FILE = os.environ.get("XSCAN_FMP_LOG", os.path.join(DSDPLUS_ROOT, "startup", "fmp24_scan.log"))
OUTPUT_DIR = os.environ.get("XSCAN_RECORDINGS_DIR", os.path.join(DSDPLUS_ROOT, "recordings"))
LOCAL_FFMPEG_EXE = os.environ.get("XSCAN_FFMPEG_EXE", os.path.join(SCRIPT_DIR, "ffmpeg", "ffmpeg.exe"))
LOCAL_FFMPEG_DIR = os.path.dirname(LOCAL_FFMPEG_EXE)
RECORDINGS_LOG_FILE = os.path.join(OUTPUT_DIR, "recordings_log.json")
SETTINGS_FILE = os.environ.get("XSCAN_LEGACY_SETTINGS", os.path.join(SCRIPT_DIR, "scanner_gui_recorder_settings.json"))
ICON_FILE = os.path.join(SCRIPT_DIR, "app.ico")
WEB_UI_PORT = int(os.environ.get("XSCAN_WEB_PORT", "8890"))
...
tuning_regex = re.compile(
    r"^Tuning to\s+([\d.]+)\s+(\S+)\s+"
    r"(?:(?:BW|DELAY)=[\d.]+\s+)*(.*?)\s*$"
)
