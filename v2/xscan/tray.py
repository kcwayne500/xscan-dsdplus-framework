...
def run_tray(context: AppContext, request_exit: Callable[[], None]) -> None:
    ...
    menu = pystray.Menu(
        ...
        pystray.MenuItem("Start Feed 1", background(context.feeds.get("feed1").runtime.start)),
        pystray.MenuItem("Stop Feed 1", background(context.feeds.get("feed1").runtime.stop)),
        pystray.MenuItem("Restart Feed 1", background(context.feeds.get("feed1").runtime.restart)),
        pystray.MenuItem("Start Feed 2", background(context.feeds.get("feed2").runtime.start)),
        pystray.MenuItem("Stop Feed 2", background(context.feeds.get("feed2").runtime.stop)),
        pystray.MenuItem("Restart Feed 2", background(context.feeds.get("feed2").runtime.restart)),
        pystray.Menu.SEPARATOR,
