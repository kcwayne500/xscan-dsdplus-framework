import { api, ApiError, feedPath, currentFeed } from './api.js?v=11';
import { mountFeeds } from './feeds.js?v=11';
...
function connectEvents() {
  ...
  const source = new EventSource(feedPath('/api/v1/events')); state.eventSource = source;
  let scheduled;
...
function showCallDetails(call) {
  ...
  el('dialogCallDownload').href = feedPath(`/api/v1/calls/${call.id}/audio?download=true`);
...
document.addEventListener('click', async event => {
  ...
  try {
    ...
    else if (action==='save-settings') { await api.saveSettings({...currentFeed()==='feed1'?{server:{session_hours:Number(el('sessionHours').value)},streaming:{enabled:el('streamEnabled').checked}}:{},runtime:{auto_restart:el('autoRestart').checked,hide_native_windows:el('hideWindows').checked},storage:{warning_free_gb:Number(el('diskWarning').value)}}); toast('Settings saved'); }
